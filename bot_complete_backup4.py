...
def cleanup_duplicates():
    ...
    try:
        # Находим дубликаты
        duplicates = db_session.query(Reward.title).group_by(Reward.title).having(db_session.func.count(Reward.id) > 1).all()
        
        for (title,) in duplicates:
            # Находим все награды с этим названием
            rewards_with_same_title = db_session.query(Reward).filter(Reward.title == title).order_by(Reward.id.desc()).all()
            
            # Оставляем первую (самую новую) и удаляем остальные
            for reward in rewards_with_same_title[1:]:
                db_session.delete(reward)
        
        db_session.commit()
        logging.info("Duplicates cleanup completed")
